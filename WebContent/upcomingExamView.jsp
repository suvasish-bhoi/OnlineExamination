...
List<ExamInstituteVO> upcomingExamList = (List<ExamInstituteVO>)request.getAttribute("UpcomingExams");
int i=0;
...
<table cellpadding="0" cellspacing="0" border="0">
  <tbody>
<% for(ExamInstituteVO eiVo : upcomingExamList){
	%><tr>
		<td><%=++i%></td>
		<td><%=eiVo.getName()%></td>
		<td><%=eiVo.getTopic()%></td>
		<td><%=eiVo.getDateOfExam()%></td>
		<td><%=eiVo.getDuration() %></td>
		<td><%=eiVo.getDescription()%></td>
		<td><% if(StudentExamMapDAO.isApplied(eiVo.getExam_id(),svo.getStudent_id())){
			out.print("Applied");
			}else{%><a href="ApplyForExamController?exam_id=<%=eiVo.getExam_id()%>&student_id=<%=svo.getStudent_id()%>" style="color: black;">Apply</a>
			<%}%></td>
		</tr>
<%
}
