...
<%@page import="com.oec.dao.ExamDAO"%>
<%@page import="com.oec.vo.StudentMarkVO"%>
<%@ page errorPage="index.jsp" %>
...
$(document).ready(function() {
    $('#Popup').click(function() {
        var newwindow = window.open($(this).prop('href'), '', 'height=800,width=900');
        if (window.focus) {
            newwindow.focus();
        }
        return false;
...
<section>
	<div  class="tbl-header">
		<table cellpadding="0" cellspacing="0" border="0">
  			<thead>
<%int exam_id = Integer.parseInt(request.getAttribute("exam_id").toString());
%>
		<tr>
		<td colspan="4">
			Topic :<%=ExamDAO.getTopicFromExamId(exam_id)%><br>
			Total Question :<%=ConductExamDAO.getQuestionCount(exam_id) %><br>
			Total Mark :<%=ExamDAO.getSumMarkbyExamId(exam_id)%>
		</td>
	</tr>
	<tr>
		<th>Sr No.</td>
	...
	<%List<StudentMarkVO> studentResultList = (List<StudentMarkVO>)request.getAttribute("studentMarkList");
...
<table cellpadding="0" cellspacing="0" border="0">
  <tbody>
	<%
	int i=0;
	for(StudentMarkVO smVo: studentResultList){
		%>
		<tr>
			<td><%=++i %></td>
			<td><%=smVo.getName()%></td>
			<td><%=smVo.getMark()%></td>
			<td><a href="CheckStudentResultController?student_id=<%=smVo.getStudent_id()%>&exam_id=<%=exam_id %>" style="color: black" id="Popup">Details</a></td>
		</tr>
	<%
	}
	%>
